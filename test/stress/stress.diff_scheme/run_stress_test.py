...
tbl = at.Table(names=('diff_scheme', 'order', 'runtime'),
               dtype=(str, int, float))

for diff_scheme in DIFF_SCHEMES:

    with tempfile.NamedTemporaryFile(mode='w+t') as f:

        # Create the input file

        f.write(template.format(diff_scheme=diff_scheme))
        f.flush()
        ...
        tbl.add_row([diff_scheme, ORDERS[diff_scheme], np.mean(times)])
