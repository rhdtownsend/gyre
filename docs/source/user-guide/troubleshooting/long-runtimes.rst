...
single :nml:group:`mode` namelist group can be approximated by
...
options in the :nml:group:`scan` namelist group(s). The
:nml:option:`n_freq` option obviously has an impact on :math:`\tau`,
as it directly sets :math:`M`. However, the :nml:option:`freq_min` and
:nml:option:`freq_max` options also influence :math:`\tau`, due to the
way the spatial grid is constructed. If the frequency scan includes
parts of the star's oscillation spectrum containing modes with very
large radial orders (whether p modes or g modes), then GYRE's
:ref:`iterative refinement algorithm <spatial-grids-iter>` will insert
many grid points in order to resolve the modes' wavefunctions. This
can ultimately lead to huge :math:`N` and very long runtimes.
...
process a single :nml:group:`orbit` namelist group can be approximated by
