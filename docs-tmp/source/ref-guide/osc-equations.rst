.. _osc-eqns:

*********************
Oscillation Equations
*********************

This chapter outlines how the oscillation equations solved by the GYRE
frontends are obtained from the basic equations of stellar structure.

.. toctree::

   osc-equations/fluid-equations.rst
   osc-equations/equilib-state.rst
   osc-equations/linear-equations.rst
   osc-equations/sep-equations.rst
   osc-equations/bound-conditions.rst
   osc-equations/dimless-form.rst
   osc-equations/rot-effects.rst
   osc-equations/conv-effects.rst
   osc-equations/tidal-effects.rst
   
