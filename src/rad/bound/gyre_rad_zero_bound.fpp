...
module gyre_rad_zero_bound
  ...
  type, extends (bound_t) :: rad_zero_bound_t
     private
  ...
  end type rad_zero_bound_t
  
  ! Interfaces

  interface rad_zero_bound_t
     module procedure rad_zero_bound_t_
  end interface rad_zero_bound_t
  
  ! Access specifiers

  private

  public :: rad_zero_bound_t

  ! Procedures

contains

  function rad_zero_bound_t_ (ml, jc, mp) result (bd)

    class(model_t), pointer, intent(in) :: ml
    class(jacobian_t), intent(in)       :: jc
    type(modepar_t), intent(in)         :: mp
    type(rad_zero_bound_t)              :: bd

    ! Construct the rad_zero_bound_t
  ...
  end function rad_zero_bound_t_

!****

  function inner_bound_ (this, x_i, omega) result (B_i)

    class(rad_zero_bound_t), intent(in) :: this
...
end module gyre_rad_zero_bound
