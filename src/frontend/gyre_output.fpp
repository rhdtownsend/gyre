...
contains
  ...
  subroutine write_summary (up, md)
    ...
    item_loop : do i = 1, SIZE(items)
       ...
       case ('f_T')
          call wr%write('f_T', [(ABS(md(j)%delT_eff()/md(j)%xi_r_ref()), j=1,n_md)])
       case ('f_g')
          call wr%write('f_g', [(ABS(md(j)%delg_eff()/md(j)%xi_r_ref()), j=1,n_md)])
       case ('psi_T')
          call wr%write('psi_T', [(phase(md(j)%delT_eff()/md(j)%xi_r_ref()), j=1,n_md)])
       case ('psi_g')
          call wr%write('psi_g', [(phase(md(j)%delg_eff()/md(j)%xi_r_ref()), j=1,n_md)])
       case('beta')
       ...
       case('omega_im')
          call wr%write('omega_im', [(md(j)%omega_im(), j=1,n_md)])
       case ('xi_r_ref')
