...
! Notes   : This code adopts the convention that perturbations are proportional to
!           exp[i(m phi - omega t). Thus:
!             - prograde modes have m*omega > 0 and m*nu > 0
!             - the Hough functions reduce to spherical harmonics in the no-rotating limit
  ...
  subroutine eigen_A_ (nu, m, parity, n, lambda, b)
    ...
    if (A_5(1) == 0._WP) then
       ...
       if (PRESENT(b)) then
          ...
          if (n > 1) call LA_STEVR(A_D(2:), A_E(2:), lambda(:n-1), Z=b(2:,:n-1))
          ...
          if (n > 1) call LA_STEVR(A_D(2:), A_E(2:), lambda(:n-1))
    ...
    elseif (A_2d(1) == 0._WP .AND. A_2n(1) /= 0._WP) then
       ...
       if (PRESENT(b)) then
          ...
          if (n > 1) call LA_STEVR(A_D(2:), A_E(2:), lambda(2:), Z=b(2:,2:))
          ...
          if (n > 1) call LA_STEVR(A_D(2:), A_E(2:), lambda(2:))
    ...
    elseif (A_3d(n) == 0._WP .AND. A_3n(n) /= 0._WP) then
       ...
       if (PRESENT(b)) then
          ...
          if (n > 1) call LA_STEVR(A_D(:n-1), A_E(:n-1), lambda(2:), Z=b(:n-1,2:))
          ...
          if (n > 1) call LA_STEVR(A_D(:n-1), A_E(:n-1), lambda(2:))
...
!****
  ...
  function N_lm_series (n, m) result (N_lm)
    ...
    ! factors N_lm, with l=|m|,|m|+1,...,|m|+n-1 (neglecting the
    ! sqrt(1/2pi) factor associated with the phi integral)
