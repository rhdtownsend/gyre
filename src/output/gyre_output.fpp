  ...
  subroutine write_mode (md, j, ot_p)
    ...
    associate (s => md%s, x => md%x, &
      ...
      item_loop : do i = 1, SIZE(items)
         ...
         case ('F_j_wave')
            call wr%write('F_j_wave', md%F_j_wave(s, x))
         case ('tau_j_wave')
            call wr%write('tau_j_wave', md%tau_j_wave(s, x))
         case ('tau_j_grow')
            call wr%write('tau_j_grow', md%tau_j_grow(s, x))
         case ('tau_j_grav')
            call wr%write('tau_j_grav', md%tau_j_grav(s, x))
         case ('Yt_1')
