...
module gyre_evol_seg
  ...
  type, extends(model_seg_t) :: evol_seg_t
     private
     $DATA_DECL(V_2)
     $DATA_DECL(As)
     $DATA_DECL(U)
...
contains
  ...
  function evol_seg_t_(x) result(sg)
...
!****
  ...
  function m_1_(this, x) result(m)

    class(evol_seg_t), intent(in) :: this
    real(WP), intent(in)          :: x
    real(WP)                      :: m

    ! Calculate the mass coordinate

    m = this%M_star*(x**3*/this%c_1(x))
  ...
  subroutine scaffold_(this, x)
    ...
    ! Return the scaffold for grid creation
