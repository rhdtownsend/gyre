...
module gyre_qho_eqns
  ...
  type, extends (r_eqns_t) :: qho_eqns_t
     private
     real(WP) :: c
   contains
     private
  ...
  interface qho_eqns_t
     module procedure qho_eqns_t_
  end interface qho_eqns_t
...
contains
  ...
  function qho_eqns_t_ (c) result (eq)

    real(WP), intent(in)   :: c
    type(qho_eqns_t)       :: eq

    ! Construct the qho_eqns_t
  ...
  end function qho_eqns_t_

!****

  function A_ (this, x, omega) result (A)

    class(qho_eqns_t), intent(in) :: this
    real(WP), intent(in)          :: x
