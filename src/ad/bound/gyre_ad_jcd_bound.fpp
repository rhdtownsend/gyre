...
module gyre_ad_jcd_bound
  ...
  type, extends (bound_t) :: ad_jcd_bound_t
     private
  ...
  end type ad_jcd_bound_t

  ! Interfaces

  interface ad_jcd_bound_t
     module procedure ad_jcd_bound_t_
  end interface ad_jcd_bound_t

  ! Access specifiers

  private

  public :: ad_jcd_bound_t

  ! Procedures

contains

  function ad_jcd_bound_t_ (ml, jc, mp) result (bd)

    class(model_t), pointer, intent(in) :: ml
    class(jacobian_t), intent(in)       :: jc
    type(modepar_t), intent(in)         :: mp
    type(ad_jcd_bound_t)                :: bd

    ! Construct the ad_jcd_bound_t
