! Incfile  : gyre_vnad_bound
! Purpose  : viscous nonadiabatic boundary conditions
!
...
module gyre_vnad_bound
  ...
  type, extends (c_bound_t) :: vnad_bound_t
     private
  ...
  end type vnad_bound_t

  ! Interfaces

  interface vnad_bound_t
     module procedure vnad_bound_t_
  end interface vnad_bound_t
...
contains
  ...
  function vnad_bound_t_ (cx, md_p, os_p) result (bd)
  ...
  end function vnad_bound_t_

  !****

  subroutine stencil_ (this, pt_i, pt_o)

    class(vnad_bound_t), intent(inout) :: this
  ...
  subroutine build_i (this, st, B, scl)
    ...
    class(vnad_bound_t), intent(in) :: this
