! Module   : gyre_nad_zero_bound 
! Purpose  : nonadiabatic boundary conditions (zero-pressure surface)
!
...
module gyre_nad_zero_bound
  ...
  type, extends (bound_t) :: nad_zero_bound_t
     private
  ...
  end type nad_zero_bound_t

  ! Interfaces

  interface nad_zero_bound_t
     module procedure nad_zero_bound_t_
  end interface nad_zero_bound_t

  ! Access specifiers

  private

  public :: nad_zero_bound_t

  ! Procedures

contains

  function nad_zero_bound_t_ (ml, jc, mp) result (bd)

    class(model_t), pointer, intent(in) :: ml
    class(jacobian_t), intent(in)       :: jc
    type(modepar_t), intent(in)         :: mp
    type(nad_zero_bound_t)              :: bd

    ! Construct the nad_zero_bound_t
  ...
  end function nad_zero_bound_t_

!****

  function inner_bound_ (this, x_i, omega) result (B_i)

    class(nad_zero_bound_t), intent(in) :: this
...
end module gyre_nad_zero_bound
